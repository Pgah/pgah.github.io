# How TLS Works — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write the blog post `src/content/posts/how-tls-works.md` explaining how TLS/HTTPS works — covering the handshake, certificates, PKI trust chain, TLS 1.2 vs 1.3 comparison, attack scenarios, and practical defense guidance.

**Architecture:** Single markdown file created incrementally, section by section. No code changes to the Astro project — only a new content file. Each task appends to the file and commits.

**Tech Stack:** Astro v6 MDX content collection, markdown, `src/content/posts/` directory.

## Global Constraints

- File: `src/content/posts/how-tls-works.md`
- Frontmatter format must match existing posts (title, date, description, tags as arrays)
- No code blocks — only inline `code` for commands, filenames, protocol values
- No diagrams — prose only
- Bold for key terms on first introduction only
- Target length: 1000–1200 words
- Tone: direct, no fluff — same register as `how-ssh-works.md`
- Date in frontmatter: `2026-07-20`
- TLS 1.3 differences noted inline in The Handshake; structural changes consolidated in "TLS 1.3: What Changed" section

---

### Task 1: Create the file — frontmatter + opening hook

**Files:**
- Create: `src/content/posts/how-tls-works.md`

**Interfaces:**
- Produces: file with valid frontmatter + 2-paragraph hook that subsequent tasks append to

- [ ] **Step 1: Write the file**

Write `src/content/posts/how-tls-works.md` with this exact content:
...
Open `src/content/posts/how-tls-works.md` and confirm:
- `title`, `date`, `description`, `tags` fields are present
- `tags` is an array (square brackets)
- `date` is `2026-07-20`

- [ ] **Step 3: Commit**

```
git add src/content/posts/how-tls-works.md
git commit -m "feat: How TLS Works blog yazısı başlatıldı (frontmatter + açılış)"
```

---

### Task 2: Add "What TLS Actually Is" and "The Problem It Solves"

**Files:**
- Modify: `src/content/posts/how-tls-works.md`

**Interfaces:**
- Consumes: file from Task 1
- Produces: two complete sections establishing what TLS is and the three problems it solves

- [ ] **Step 1: Append "What TLS Actually Is" section**
...
- [ ] **Step 2: Append "The Problem It Solves" section**
...
Read the file and confirm it now has three content blocks: opening hook, "What TLS Actually Is", "The Problem It Solves".

- [ ] **Step 4: Commit**

```
git add src/content/posts/how-tls-works.md
git commit -m "feat: TLS tanımı ve çözdüğü problem bölümleri eklendi"
```

---

### Task 3: Add "The Handshake" section

**Files:**
- Modify: `src/content/posts/how-tls-works.md`

**Interfaces:**
- Consumes: file from Task 2
- Produces: The Handshake section — TLS 1.2 baseline with TLS 1.3 differences noted inline at each step

- [ ] **Step 1: Append "The Handshake" section**
...
- [ ] **Step 2: Verify inline TLS 1.3 notes are present**

Confirm the section mentions TLS 1.3 differences at ClientHello, ServerHello, and Key Exchange steps — not only in a separate section.

- [ ] **Step 3: Commit**

```
git add src/content/posts/how-tls-works.md
git commit -m "feat: TLS handshake bölümü eklendi (1.2 baseline, 1.3 karşılaştırması inline)"
```

---

### Task 4: Add "The Certificate" and "The Trust Chain" sections

**Files:**
- Modify: `src/content/posts/how-tls-works.md`

**Interfaces:**
- Consumes: file from Task 3
- Produces: two sections explaining what certificates contain and how the CA trust hierarchy works

- [ ] **Step 1: Append "The Certificate" section**
...
- [ ] **Step 2: Append "The Trust Chain" section**
...
- [ ] **Step 3: Verify DigiNotar incident is mentioned**

The Trust Chain section must include the DigiNotar example — it's the spec's specified illustration of where trust breaks.

- [ ] **Step 4: Commit**

```
git add src/content/posts/how-tls-works.md
git commit -m "feat: sertifika yapısı ve güven zinciri bölümleri eklendi"
```

---

### Task 5: Add "TLS 1.3: What Changed" section

**Files:**
- Modify: `src/content/posts/how-tls-works.md`

**Interfaces:**
- Consumes: file from Task 4
- Produces: section consolidating structural TLS 1.3 changes (1-RTT, 0-RTT, forward secrecy, removed ciphers, downgrade protection)

- [ ] **Step 1: Append "TLS 1.3: What Changed" section**
...
- [ ] **Step 2: Verify all five changes are covered**

The section must mention: 1-RTT, 0-RTT, forward secrecy mandatory, weak algorithms removed, downgrade protection.

- [ ] **Step 3: Commit**

```
git add src/content/posts/how-tls-works.md
git commit -m "feat: TLS 1.3 değişiklikleri bölümü eklendi"
```

---

### Task 6: Add "What Can Go Wrong" and "What Good TLS Looks Like"

**Files:**
- Modify: `src/content/posts/how-tls-works.md`

**Interfaces:**
- Consumes: file from Task 5
- Produces: attack scenarios and practical defensive checklist sections

- [ ] **Step 1: Append "What Can Go Wrong" section**
...
- [ ] **Step 2: Append "What Good TLS Looks Like" section**
...
- [ ] **Step 3: Verify all four attack types are in "What Can Go Wrong"**

Confirm: downgrade attacks, rogue certificates, expired certificates, MITM with installed root.

- [ ] **Step 4: Verify all five checklist items are in "What Good TLS Looks Like"**

Confirm: version, cipher suites, certificate, HSTS, OCSP stapling.

- [ ] **Step 5: Commit**

```
git add src/content/posts/how-tls-works.md
git commit -m "feat: saldırı senaryoları ve iyi TLS konfigürasyonu bölümleri eklendi"
```

---

### Task 7: Add closing paragraph and final verification

**Files:**
- Modify: `src/content/posts/how-tls-works.md`

**Interfaces:**
- Consumes: file from Task 6 (all sections complete)
- Produces: finished post ready for publication

- [ ] **Step 1: Append closing paragraph**
...
Run: `wc -w src/content/posts/how-tls-works.md`

Expected: between 950 and 1300 words (frontmatter excluded). If significantly under 950, a section may have been missed.

- [ ] **Step 3: Section heading check**

Confirm the file contains exactly these headings in order:
1. `## What TLS Actually Is`
2. `## The Problem It Solves`
3. `## The Handshake`
4. `## The Certificate`
5. `## The Trust Chain`
6. `## TLS 1.3: What Changed`
7. `## What Can Go Wrong`
8. `## What Good TLS Looks Like`

- [ ] **Step 4: Code block check**

Run: `grep '^\`\`\`' src/content/posts/how-tls-works.md`

Expected: no output. Per spec, no fenced code blocks — only inline backtick code.

- [ ] **Step 5: Final commit**

```
git add src/content/posts/how-tls-works.md
git commit -m "feat: How TLS Works blog yazısı tamamlandı"
```
