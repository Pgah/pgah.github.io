...
const CELL = 'display:inline-block;width:3ch;text-align:center;font-weight:bold;margin-right:0.3ch';

export function formatRow(guess: string, result: GuessResult[]): string {
  return guess.toUpperCase().split('').map((ch, i) => {
    const r = result[i];
    const bg = r === 'correct' ? 'var(--green)' : r === 'present' ? 'var(--fg-bright)' : 'var(--fg-muted)';
    return `<span style="${CELL};background:${bg};color:var(--bg)">${ch}</span>`;
  }).join('');
}

const EMPTY_ROW = Array(5).fill(null).map(() =>
  `<span style="${CELL};background:var(--bg-soft);color:var(--fg-muted)">·</span>`
).join('');

export function formatBoard(state: WordleState): string[] {
  return Array.from({ length: 6 }, (_, i) =>
    i < state.guesses.length
      ? formatRow(state.guesses[i], state.results[i])
      : EMPTY_ROW
  );
}
...
export function buildShareText(state: WordleState, day: number): string {
  ...
  return [header, '', ...rows, '', 'matrix-303.pages.dev'].join('\n');
}
